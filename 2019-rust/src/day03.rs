...
fn parse(inp: &str) -> Vec<Vec<&str>> {
    inp.lines().map(|s| s.split(',').collect()).collect()
}
...
fn intersections(wires: &[HashMap<Point, u64>]) -> HashSet<Point> {
    let mut wire_coords_iter = wires.iter().map(|wire| to_keys(&wire));

    let first_wire_coords = wire_coords_iter.next().unwrap();
    wire_coords_iter.fold(first_wire_coords, |acc, x| {
        acc.intersection(&x).cloned().collect()
    })
}

fn to_keys<T: std::clone::Clone + std::cmp::Eq + std::hash::Hash, U>(
    map: &HashMap<T, U>,
) -> HashSet<T> {
...
fn _part2(inp: &str, _sample: bool) -> String {
    ...
    intersections(&wires[..])
        ...
        .map(|coord| {
            wires
                .iter()
                .map(|wire| wire.get(&coord).unwrap())
                .sum::<u64>()
        })
        .filter(|&i| i != 0)
        .min()
