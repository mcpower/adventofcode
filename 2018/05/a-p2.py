...
def react(inp):
    ...
    while True:
        i = 0
        while i < len(inp):
            if i + 1 < len(inp) and inp[i].lower() == inp[i+1].lower() and inp[i] != inp[i+1]:
                i += 2
            else:
                new_string += inp[i]
                i += 1
...
def do_case(inp: str, sample=False):
    ...
    import string
    print(min(react(inp.replace(i, "").replace(i.upper(), "")) for i in string.ascii_lowercase))
